...
ROOT=/srv/haze-weather-radio
STAGE=${1:-/home/rai/haze-deploy-input/climate-location-20260811}

if [[ ${EUID:-1} -ne 0 ]]; then
    printf 'Run this deployment script through sudo.\n' >&2
    exit 1
fi
[[ "$(realpath "$ROOT")" == /srv/haze-weather-radio ]]
...
backup="$ROOT/deployment-backups/pre-climate-location-$stamp"
...
systemctl restart haze-weather-radio.service
systemctl is-active --quiet haze-weather-radio.service
